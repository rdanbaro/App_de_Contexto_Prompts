from Vistas.ui_vista_registro import Ui_VistaRegistro
from PySide6.QtWidgets import QWidget, QMessageBox, QLineEdit
import database
from Modelo.modelos import Usuario

# Clase ControladoraRegistro que hereda de QWidget y Ui_VistaRegistro
    ...
    def register_validator(self):
        ...
        try:
            if usuario == '' or contrasena == '' or confirmar_contrasena == '':
                ...
                self.label_error_visual_reg.setText('                        Faltan campos por completar!')
                cuenta_valida = False
            elif len(usuario) > 50:
                print('nombre de usuario demasiado largo')
                self.label_error_visual_reg.setText('                        nombre de usuario demasiado largo')
                cuenta_valida = False
                ...
                self.label_error_visual_reg.setText('                        contrasena demasiado larga')
                cuenta_valida = False
            elif contrasena != confirmar_contrasena:
                print("La confirmacion y la contrasena no coinciden")
                self.label_error_visual_reg.setText(' La confirmacion y la contrasena no coinciden')
                cuenta_valida = False
            if cuenta_valida:
            ...
            self.label_error_visual_reg.setText('Algun error ha ocurrido, verifique que los campos sean validos')
            cuenta_valida = False
